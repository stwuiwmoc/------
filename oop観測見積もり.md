...
ここで、波長積分の中身では、分光放射輝度 $I _{all} $ から検出器の1pixelに到達する光子数を計算し、量子効率 $\eta $ （1個の光子が何個の電子に変換されるか）を掛けて検出器が出力する電流（=電子数）を求めている。検出器が出力する電流には、光 $I _{all} $ による電子に加えて検出器暗電流 $I _{dark} $ による電子も含まれる。これらの電子はフォトダイオードに蓄積され、その蓄積量は積分時間 $t _{obs} $ に応じて増加する。

蓄積された電子は $t _{obs} $ の経過後に電流として読み出され、検出器読み出し回路によってカウント値に変換される。電子数からカウント値への変換は、電子数をシステムゲイン $G _{sys}$ で割ることで求められる。
...
となる。この式の第一項は検出器に到達した分光放射輝度によるカウント値、第二項は暗電流によるカウント値、第三項は読み出しノイズによるカウント値であるから
