...
| $t _{obs} $        | s                    | 積分時間                                     |        | 任意       |
| $N_ {e.read} $     | e $^- _{rms} $ / pix | 駆動回路読み出しノイズ                       | 1200   | 想定値     |
...
また、検出器読み出し回路によるカウント値への変換の過程で加わるノイズのRMSを電子数に換算したものが読み出しノイズ $N_ {e.read} $ である。このため、最終的に得られるカウント値 $S _{all.pix}$ は、光 $I' _{all} $ によるカウント値と暗電流 $I _{dark}$ によるカウント値に、読み出しノイズ $N_ {e.read}$ によるカウント値を加えたものとなる。
