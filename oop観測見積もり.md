...
[Edmund gold coating]: https://www.edmundoptics.jp/knowledge-center/application-notes/optics/metallic-mirror-coatings/

- [注釈](#注釈)
- [見積もりの概略](#見積もりの概略)
  - [見積もりで想定する物理過程](#見積もりで想定する物理過程)
  - [シミュレーション上での分光放射輝度の扱い](#シミュレーション上での分光放射輝度の扱い)
  - [ジャンスキー（Jy）から分光放射輝度への換算](#ジャンスキーjyから分光放射輝度への換算)
...
- [近赤外装置による撮像・分光](#近赤外装置による撮像分光)
  ...
  - [NDフィルタによる透過率の導出](#ndフィルタによる透過率の導出)
  - [装置透過率の導出（TOPICS）](#装置透過率の導出topics)
  - [装置透過率の導出（ESPRIT 撮像モード）](#装置透過率の導出esprit-撮像モード)
  - [装置透過率の導出（ESPRIT 分光モード）](#装置透過率の導出esprit-分光モード)
...
観測対象の発光に対して、大気や光学系による強度減衰と、sky backgroundや望遠鏡自体の発光が加わりながら検出器に到達し、装置ノイズを含んだカウント値として出力されることを想定する。輝線発光やsky backgroundには波長依存性があるので、波長方向に1次元の分光放射輝度を考え、検出器に到達した段階で波長方向に積分することを想定する。
...
| $\nu$          | Hz                  | 観測対象の周波数                          |
| $F$            | W / m $^2$          | 放射照度                                  |
| $F _{\lambda}$ | W / m $^2$ / m      | 分光放射照度（=単位波長当たりの放射照度） |
| $F _{\nu}$     | W / m $^2$ / Hz     | 単位周波数当たりの放射照度                |
| $c$            | m / s               | 光速                                      |
|                |                     |                                           |
| $J _{sr}$      | Jy / sr             | 単位立体角当たりのジャンスキー            |
| $I' _{}$       | W / m $^2$ / sr / m | 分光放射輝度                              |
|                |                     |                                           |

ジャンスキー [Jy] は電波天文学の分野で使われる単位で、1 [Jy] は1 [m $^2$ ] の面積に 1 [Hz] あたりのエネルギーの流量が 10^-26 [W] であるときの電波放射の強さと定義されている。このため、

1 [Jy] = 10 $^{-26}$ [W / m $^2$ / Hz]

であり、単位周波数当たりの放射照度を電波天文学で使いやすいようにスケーリングしたものだと考えれば良い。よってジャンスキーから分光放射輝度への変換には、単位周波数当たりの放射照度 $F _{\nu}$ から 分光放射輝度 $F _{\lambda}$ への変換を考える必要がある。

まず、$c = \nu \lambda$ より $\nu = c / \lambda$ である。これを波長で微分すると

$$
\cfrac{d \nu}{d \lambda} =  - \cfrac{c}{\lambda ^2}
$$

となる。ここで、符号の - は、波長が大きくなると周波数が小さくなることに対応しているが、ここでは絶対値のみを考えたいため無視する。

次に、$F _{\nu}$ と $F _{\lambda}$ はどちらも 単位波長 or 単位周波数 当たりの放射照度であるため、以下の等式が成り立つ。

$$
F = F _{\lambda} \cdot d \lambda = F _{\nu} \cdot d \nu
$$

よって上式を変形すると

$$
\cfrac{F _{\lambda}}{F _{\nu}} = \cfrac{d \nu}{d \lambda} = \cfrac{c}{\lambda ^2}
$$

となる。さらに上式を変形すると

$$
F _{\lambda} = \cfrac{c}{\lambda ^2} \cdot F _{\nu}
$$

と整理できる。よって、例えば単位立体角当たりのジャンスキー $J _{sr}$ [Jy / sr] (=[W / m $^2$ / sr / Hz]) を分光放射輝度 $I' _{}$ [W / m $^2$ / sr / m]に変換する場合

$$
I' _{} = 10 ^{-26} \cdot \cfrac{c}{\lambda ^2} \cdot J _{sr}
$$
...
|         |      | クーデ                     | F24 | 0.8 $^5$            | 鍵谷先生 |
|         |      | クーデ＋イメージローテータ | F24 | 0.8 $^8$            | 鍵谷先生 |
| Pirika  | 1.6  | カセグレン                 |     |                     |          |
|         |      | ナスミス                   | F12 | 0.88 $^3$（暫定値） |          |
| PLANETS | 1.8  | グレゴリアン               |     |                     |          |
|         |      | クーデ                     | F63 | 0.8 $^6$            | 鍵谷先生 |
|         |      | クーデ＋前置光学系         | F12 | 0.8 $^7$            | 鍵谷先生 |
|         |      | クーデ＋AO＋前置光学系     | F12 | 0.8 $^8$            | 鍵谷先生 |

PLANETSのクーデ焦点のパラメータは、2022年6月時点でのパラメータである。（F値に関しては前置光学系で変換できるので問題ない）
...
TOPICS, ESPRITのフィルターターレットは2段構造になっており、1段目はバンドパスフィルタの設置を想定している。
バンドパスフィルタは、TOPICSでは波長域選択のために、ESPRITではオーダーソートフィルターとしてクロスディスパーザーを代替するために用いられる。

| 文字                    | 単位   | 意味                       | 代入値 |
| ----------------------- | ------ | -------------------------- | ------ |
| $\lambda _{BPF.center}$ | m      | 中心波長                   | 下表   |
| $FWHM _{BPF}$           | m      | 半値全幅                   | 下表   |
| $tau _{BPF.center}$     | 無次元 | 中心波長での透過率         | 下表   |
|                         |        |                            |        |
| $tau _{i.BPF}$          | 無次元 | バンドパスフィルタの透過率 |        |
|                         |        |                            |        |

高橋M論p38で示されているように、各バンドパスフィルタの透過率は非線形な波長依存性と温度依存性を持つが、見積もりではガウシアンとして扱う。この場合、 $tau _{i.BPF}$ は波長 $\lambda$ の関数として

$$
tau _{i.BPF} (\lambda)
= tau _{BPF.center} \cdot
  \exp{
    \left (
      - \cfrac
        {(\lambda - \lambda _{BPF.center})^2}
        {2 \left( \cfrac{FWHM _{BPF}}{2 \sqrt{2 \ln 2}} \right)^2}
    \right )
  }
$$
...
| $\lambda _{BPF.center}$ | $FWHM _{BPF}$ | $tau _{BPF.center}$ | 設置先 | 参照元             |
| ----------------------- | ------------- | ------------------- | ------ | ------------------ |
...
TOPICS, ESPRITのフィルターターレットは2段構造になっており、2段目はNDフィルタの設置を想定している。
理想的なNDフィルタは全ての光を波長依存せずに一様に減光するため、明るい観測対象に対して短い積分時間で検出器が飽和しないようにすることを意図している。

| 文字           | 単位   | 意味                        | 代入値 |
| -------------- | ------ | --------------------------- | ------ |
| $OD$           | 無次元 | 光学濃度（Optical Density） | 下表   |
|                |        |                             |        |
| $\tau _{i.ND}$ | 無次元 | NDフィルタの透過率          |        |
|                |        |                             |        |


NDフィルタの透過率 $\tau _{i.ND}$ は光学濃度 $OD$ を用いて

$$
\tau _{i.ND} = 10 ^{-OD}
$$

と計算される。NDフィルタを用いない場合は $\tau _{i.ND} = 1$ とすればよい。

以下で現在設置されているNDフィルタの可視光帯での $OD$ を示した。但し、例えばLバンドなどの近赤外帯では減光の効率が悪化するため、 $\tau _{i.ND}$ はおよそ1桁程度大きくなる。

| $OD$ | $\tau _{ND}$ | 設置先 | 参照元     |
| ---- | ------------ | ------ | ---------- |
| 0.5  | $10 ^{-0.5}$ | TOPICS | 神原M論p26 |
| 1.5  | $10 ^{-1.5}$ | TOPICS | 神原M論p26 |
| 2.5  | $10 ^{-2.5}$ | TOPICS | 神原M論p26 |
|      |              |        |            |
...
| 文字               | 単位   | 意味                       | 代入値    | 参照元                                 |
| ------------------ | ------ | -------------------------- | --------- | -------------------------------------- |
| $\tau _{i.lens}$   | 無次元 | 内部光学系のレンズの透過率 | 0.9 $^2$  | 高橋M論p44                             |
| $\tau _{i.window}$ | 無次元 | 入光窓の透過率             | 0.9       | 高橋M論p44                             |
| $\tau _{i.mirror}$ | 無次元 | 内部光学系の鏡の透過率     | 1（無視） | 高橋M論p98と野口M論p93で無視されている |
| $\tau _{i.BPF}$    | 無次元 | バンドパスフィルタの透過率 | 導出済    |                                        |
| $\tau _{i.ND}$     | 無次元 | NDフィルタの透過率         | 導出済    |                                        |
|                    |        |                            |           |                                        |
...
$$
\tau _i = \tau _{i.lens} \cdot \tau _{i.window} \cdot \tau _{i.mirror} \cdot \tau _{i.BPF} \cdot \tau _{i.ND}
$$

ここで、 $\tau _{i.lens}$ , $\tau _{i.window}, \tau _{i.mirror}$ は厳密には波長依存性があるが、想定する観測波長範囲（＝CRC463の感度波長範囲）においては波長方向に一定と見為せると考えて固定値で計算する。

また、野口M論p93ではレンズの透過率について単レンズ5枚と入光窓で $0.9^6 = 0.53$ としているが、ここでは光学系周りについてより深く開発に携わった高橋M論を参考にした。（野口M論p93ではレンズの透過率を低く見積もっているが、一方でバンドパスフィルタの透過率をかけ忘れているため、装置全体の透過率としては高橋M論と大きく変わらない値となっている。）

## 装置透過率の導出（ESPRIT 撮像モード）

| 文字                | 単位   | 意味                             | 代入値    | 参照元     |
| ------------------- | ------ | -------------------------------- | --------- | ---------- |
| $\tau _{i.mirror}$  | 無次元 | 内部光学系の鏡の反射率           | 0.96 $^9$ | 神原M論p86 |
| $\tau _{i.BPF}$     | 無次元 | バンドパスフィルタの透過率       | 導出済    |            |
| $\tau _{i.ND}$      | 無次元 | NDフィルタの透過率               | 導出済    |            |
| $\tau _{i.thermal}$ | 無次元 | サーマルカットフィルターの透過率 | 下記参照  | 神原M論p94 |
|                     |        |                                  |           |            |
| $\tau _i$           | 無次元 | 装置内部の透過率合算             |           |            |
|                     |        |                                  |           |            |

装置内部の透過率は、ESPRIT 撮像モードでは

$$
\tau _i = \tau _{i.mirror} \cdot \tau _{i.BPF} \cdot \tau _{i.ND} \cdot \tau _{i.thermal}
$$

ここで、$\tau _{i.mirror}$ の透過率は一般的な金コーティングミラーの反射率である96%としている
また、$\tau _{i.thermal}$ については、神原M論p94に記載された購入元サイトにある波長－透過率特性のデータを用いた。

## 装置透過率の導出（ESPRIT 分光モード）

| 文字                | 単位   | 意味                             | 代入値    | 参照元      |
| ------------------- | ------ | -------------------------------- | --------- | ----------- |
| $\tau _{i.mirror}$  | 無次元 | 内部光学系の鏡の反射率           | 0.96 $^8$ | 神原M論p86  |
| $\tau _{i.BPF}$     | 無次元 | バンドパスフィルタの透過率       | 導出済    |             |
| $\tau _{i.ND}$      | 無次元 | NDフィルタの透過率               | 導出済    |             |
| $\tau _{i.thermal}$ | 無次元 | サーマルカットフィルターの透過率 | 下記参照  | 神原M論p94  |
...
$$
\tau _i = \tau _{i.mirror} \cdot \tau _{i.BPF} \cdot \tau _{i.ND} \cdot \tau _{i.thermal} \cdot \tau _{i.grating}
$$
...
と整理できる。なお、カウント値はADコンバータによって電圧値から変換される時に整数として記録されるため、見積もりでは $S _{photon.pix}$ , $S _{dark.pix}$ , $S _{read.pix}$ を導出した時点で小数点以下は切り捨てている。
