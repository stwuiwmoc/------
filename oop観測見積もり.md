...
また、検出器読み出し回路によるカウント値への変換の過程で加わるノイズのRMSを電子数に換算したものが読み出しノイズ $N _{read} $ である。このため、最終的に得られるカウント値 $S _{all.pix}$ は、光 $I _{all} $ によるカウント値と暗電流 $I _{dark}$ によるカウント値に、読み出しノイズ $N _{read}$ によるカウント値を加えたものとなる。
